...
#pragma CODE_SECTION(DecimalToBinary, "ramfuncs");
void DecimalToBinary(Uint16 val, Uint16 bits, Uint16 binArr[])
{
	int i=0;
	if(val < 2)
	{
	...
	else
	{
		binArr[bits - 1] = val%2;
		val/=2;
		DecimalToBinary(val, bits-1, binArr);
	}//end else
}

#pragma CODE_SECTION(GrayToBinary, "ramfuncs");
...
#pragma CODE_SECTION(BinaryToDecimal, "ramfuncs");
...
#pragma CODE_SECTION(square, "ramfuncs");
int square(int cnt)
{
	int ret = 1;
	int i;
