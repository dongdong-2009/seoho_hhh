...
//	SysCtrlRegs.HISPCP.all = 0x0001;		// system clock out/2	150MHz/2= 75
	...
	AdcRegs.ADCTRL1.bit.SEQ_CASC = 0;// Enable SOCA from ePWM to start SEQ1
/*
...
void GetSensorValue( )
{
	...
	Ics= -(Ias+Ibs);
		...
		if (Flag.Monitoring.bit.DRIVE_CAL)
		{
			Temp= Ics;
			Ics= Ibs;
			Ibs= Temp;
...
void dev_BackgroundDAC ( void )
{
	...
	deg_pDac = &deg_Dac [0];
	Temp= (DA1-DA1_offset)/DA1_rng;
	deg_pDac->fValue= BOUND(Temp, 1, -1);
	dev_SendDAC ( deg_pDac );
	NOP; 
	deg_pDac++;
	Temp= (DA2-DA2_offset)/DA2_rng;
	deg_pDac->fValue= BOUND(Temp, 1, -1);
	dev_SendDAC ( deg_pDac );
	NOP; 
	deg_pDac++;
	Temp= (DA3-DA3_offset)/DA3_rng;
	deg_pDac->fValue= BOUND(Temp, 1, -1);
	dev_SendDAC ( deg_pDac );
	NOP; 
	deg_pDac++;
	Temp= (DA4-DA4_offset)/DA4_rng;
	deg_pDac->fValue= BOUND(Temp, 1, -1);
	dev_SendDAC ( deg_pDac );
	NOP;
...
void EPWM_Initialization_Online()
{  
	EPwmPeriodCount =	(Uint16)( F_OSC * DSP28_PLLCR / ((double)P.G01.P07_PWM_frequency_x10_kHz*1.e+2) / 2 /2);
