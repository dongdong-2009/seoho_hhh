...
void Get_Command( int * Command, double * Init_reference )
{
	...
	if ( (Digital_reference== 0.)&&(P.G03.P51_S_curve_enable== 0) )
...
void DI_Function()
{
	...
	run_chk = Flag.DI.bit.INVERTER_RUN;
			...
			switch( P.G06.AI1.P02_Type ) // input type
			{
				case 0:				// 0~10V
					...
					if( Temp4 == 0){
						...
						min = Temp2*0.01*10.0; 	// 10V 일때 100 임 
						max = Temp3*0.01*10.0;
					}
