...
UINT16 GenerateCrc( UCHAR* pabMessage, UCHAR iLength )
{
   ...
   while ( iLength )
      {
      uIndex = (UINT16) ( uchCRCHi ^ *pabMessage++ ) ;

      uchCRCHi = (unsigned char) ( uchCRCLo ^ abCrcHi[uIndex] ) ;
      uchCRCLo =  abCrcLo[uIndex];
      iLength-- ;
      }
...
UINT16 GenerateCrc( UCHAR* pabMessage, UINT16 iLength )
{
   ...
   UCHAR bByteCount;
   UCHAR bBitCount, bCarryFlag;

   for( bByteCount = 0 ; bByteCount < iLength ; bByteCount++ )
   {

      *((UCHAR*)&iCRCReg + 1) = pabMessage[ bByteCount ] ^ ( iCRCReg & 0x00FF );
