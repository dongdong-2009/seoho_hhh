...
{ 
	...
	DDRD = 0xF0;	
	PORTD = 0x00;
	DDRE = 0x00;					// PORTE7~4 = input
	PORTE = 0x00;	
	DDRF = 0xFF;         			// PORTF3~0 = output
	PORTF = 0x00;
	DDRH = 0xFF;					// PORTH = output (LCD/GLCD control)
	PORTH = 0x00;					//    * clear LCD/GLCD control signal
	DDRJ = 0xF8;					// PORTJ7-0 = input
	PORTJ = 0x00;					//    * clear
	DDRK = 0x00;                  // PORTK7-0 = input
	PORTK = 0x00;
...
int main(void)
{
	...
	GLCD_print1616(0, 3, "  -±‚º˙ø¨±∏º“-");
	//GLCD_PutStringScrollBuf("LCD TESTING NOW...");
	//LoadDiagram();
