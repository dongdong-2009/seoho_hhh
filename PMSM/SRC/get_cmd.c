...
void get_cmd(unsigned char GROUP,unsigned char INDEX)
{
	Group_index = (unsigned int)(GROUP * 100 + INDEX);
	switch(Group_index)
	{
		case 100 : 	P_rate = 100 * (float)(Data_Registers[0] = Comm_array[40]);
		 		 	break ;
		case 101 : 	V_rate = (float)(Comm_array[41]) / 10.;
		 		 	break ;
		case 102 : 	I_rate = (float)(Comm_array[42] / 10.);
		 		 	break ;
