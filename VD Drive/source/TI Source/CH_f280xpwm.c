...
{       
	p->PeriodMax =	(Uint16)( F_OSC * DSP28_PLLCR / ((double)P.G01.P07_PWM_frequency_x10_kHz*1e+2) / 2 /2);
	...
	// Setup compare 
	EPwm1Regs.CMPA.half.CMPA = p->PeriodMax;
...
//	EPwm1Regs.TZSEL.bit.CBC4= 1;
	...
	// Setup compare 
	EPwm2Regs.CMPA.half.CMPA = p->PeriodMax;
...
//	EPwm2Regs.TZSEL.bit.CBC4= 1;
	...
	// Setup compare 
	EPwm3Regs.CMPA.half.CMPA = p->PeriodMax;
...
//		unsigned int tmp4;
        ...
        EPwm1Regs.CMPA.half.CMPA = tmp1;//tmpÓÒÒÆ16±äÎª1/2,È»ºóm_periodÓÒÒÆ1±äÎª1/2
		EPwm1Regs.CMPB = tmp1;

		EPwm2Regs.TBPRD = m_period;
        tmp2 = (unsigned int)((p->Mfunc_c2  + 1)* tmp5);
		EPwm2Regs.CMPA.half.CMPA = tmp2;
		EPwm2Regs.CMPB = tmp2;

		EPwm3Regs.TBPRD = m_period;
        tmp3 = (unsigned int)((p->Mfunc_c3  + 1)* tmp5);
		EPwm3Regs.CMPA.half.CMPA = tmp3;
		EPwm3Regs.CMPB = tmp3;
